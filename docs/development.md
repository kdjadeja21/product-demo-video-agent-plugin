...
Hygiene for apps that *use* the plugin (do not vendor it, do not change UI for capture, commit config + final media, link the video in the PR/response by default) lives in [`skills/product-demo/SKILL.md`](../skills/product-demo/SKILL.md). Keep that in sync with the commit list in [`README.md`](../README.md).
